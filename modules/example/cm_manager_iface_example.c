...
*/
	cm_object_put(&self->owner->cmobj);
	free(self->priv);
	free(self);
...
/*
static struct cm_object *
	cm_manager_iface_get(struct cm_manager_iface *self)
{
	assert(self);
	cm_object_get(&self->owner->cmobj);
	return cm_object_get(&self->cmobj);
}
...
struct cm_manager_iface * cm_manager_iface_new(struct cm_module *owner)
{
